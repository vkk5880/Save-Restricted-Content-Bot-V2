"""
FastTelethon - Parallel file transfer implementation for Telethon
...
Enhanced with DC migration handling, better error recovery, and performance optimizations
"""
...
MAX_DOWNLOAD_SPEED = int(getenv("MAX_DOWNLOAD_SPEED", "15")) * 1024 * 1024  # 15 Mbps in bytes
MAX_CHUNK_SIZE = int(getenv("MAX_CHUNK_SIZE", "3072")) * 1024  # 3072KB
MIN_CHUNK_SIZE = int(getenv("MIN_CHUNK_SIZE", "128")) * 1024  # 128KB
SPEED_CHECK_INTERVAL = int(getenv("SPEED_CHECK_INTERVAL", "1"))  # Seconds between speed checks
...
MAX_MIGRATION_RETRIES = int(getenv("MAX_MIGRATION_RETRIES", "3"))
...
class DownloadSender:
    ...
    async def next(self) -> Optional[bytes]:
        ...
        try:
            # Throttle if exceeding speed limit
...
class ParallelTransferrer:
    ...
    async def _create_sender_for_dc(self, dc_id: int) -> MTProtoSender:
        ...
        # Only reset auth_key if we're changing DCs
        if self.auth_key is None or self.client.session.dc_id != dc_id:
            self.auth_key = None
    ...
    async def _create_sender(self) -> MTProtoSender:
        ...
        try:
            ...
            # Only perform auth transfer if we don't have an auth_key yet
            if self.auth_key is None:
                try:
                    logger.debug(f"Exporting auth from main DC {self.client.session.dc_id} to {dc.id}")
                    auth = await self.client(ExportAuthorizationRequest(dc.id))
                    self.client._init_request.query = ImportAuthorizationRequest(id=auth.id, bytes=auth.bytes)
                    req = InvokeWithLayerRequest(LAYER, self.client._init_request)
                    await sender.send(req)
                    self.auth_key = sender.auth_key
                    logger.debug(f"Successfully established auth for DC {dc.id}")
                except Exception as auth_error:
                    await sender.disconnect()
                    raise ConnectionError(f"Authorization failed for DC {dc.id}: {str(auth_error)}")
                    
            return sender
        except Exception as e:
            async with self._connection_lock:
                self._active_connections -= 1
            logger.error(f"Error creating sender for DC {self.dc_id}: {str(e)}")
            raise
    ...
    async def download(self, file: TypeLocation, file_size: int, part_size_kb: Optional[float] = None, connection_count: Optional[int] = None) -> AsyncGenerator[bytes, None]:
        ...
        # Get initial DC from file location
        dc_id, location = utils.get_input_location(file)
        self.dc_id = dc_id
        self.auth_key = None  # Will be generated during first connection
        
        retry_count = 0
        
        while retry_count < MAX_MIGRATION_RETRIES:
            try:
                ...
                retry_count += 1
                logger.info(f"File migrated to DC {e.new_dc}, reconfiguring...")
                await self._cleanup()
                
                # Update DC and reset auth key
                self.dc_id = e.new_dc
                self.auth_key = None
                
                # Update location with new DC info if needed
                if hasattr(location, 'dc_id'):
                    location.dc_id = e.new_dc
        ...
        raise ConnectionError(f"Failed after {MAX_MIGRATION_RETRIES} migration attempts")
